...
Aby rozpocząć pracę z aplikacją jest potrzebny serwer i sama aplikacja.
...
```
docker build -t go-expenses-server -f server.Dockerfile .
docker run -ti -p 1488:1488 go-expenses-server
```
...
Jako przykład można spojrzeć na `compose.yaml`.

### Uwagi

- Aplikacja NIE pracuje bez serwera i nie zawiera lokalnego przechowywania danych, oprócz samych ustawień użytkowanika np. adresu serwera, nazwy użytkowina i JWT klucza, jaki jest przechowiwany dla autentyfikacji z serwerem. Więc przy pojawieniu błedów powiązanych z aplikacją - nie ma potrzeby matrwić się. To nie państwa problem. A problem programisty.
...
- Modyfikacja typów tranzakacji na stronie klienta. Możliwość modyfikacji lub ją brak zależy od ustawień ze strony serwera.
- No i bugfix
